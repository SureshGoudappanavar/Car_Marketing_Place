...
function DetailHeader({ carDetail }) {
  return (
    <div>
      {carDetail?.vehicleModel ? (
        <div>
          <h2 className='font-bold text-2xl sm:text-3xl ml-4 sm:ml-16'>{carDetail.vehicleModel}</h2>
          <p className='ml-4 sm:ml-16 text-sm'>Luxury Car For Sale</p>

          <div className='flex flex-wrap gap-2 mt-3 ml-4 sm:ml-16'>
            {[
              ...
              <div key={i} className='flex gap-2 items-center bg-blue-50 rounded-full p-1 px-3'>
                ...
                <h2 className='text-primary text-sm'>{item.label}</h2>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <div className='w-full rounded-xl h-[100px] bg-slate-200 animate-pulse mt-5 mx-4 sm:mx-16'></div>
      )}
    </div>
  );
}
